...
async function createTestData() {
	...
	try {
		...
		for (const user of testUsers) {
			...
			if (error) {
				console.log(
					`⚠️  ${user.username} 생성 중 오류 (이미 존재할 수 있음):`,
					error.message,
				);
			} else {
		...
		for (const room of testRooms) {
			...
			if (error) {
				...
				console.log(
					`✅ ${room.id} 생성 완료 (멤버: ${JSON.parse(room.members).join(", ")})`,
				);
			}
		}
		...
		console.log("\n📋 테스트 가능한 기능들:");
		console.log(
			"1. 🔑 로그인: test_user1, test_user2, test_user3, test_user4 중 아무거나",
		);
		console.log("2. 💬 채팅: 3개의 테스트 채팅방 생성됨");
		console.log(
			"3. 🏆 MVP 투표: 모든 날짜의 방에서 투표 가능 (날짜 제한 해제)",
		);

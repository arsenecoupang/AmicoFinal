...
export default function MvpVoteTest(){
  ...
  useEffect(()=>{
    (async ()=>{
      const { data: p } = await supabase.from('profiles').select('id,username,realname');
      setProfiles(p || []);
  ...
  const simulateVote = async () => {
    if(!selectedProfile || !selectedCandidate) return setStatus('프로필과 후보를 선택하세요');
    try{
      setStatus('투표 중...');
      // 선택한 프로필의 id, 선택 후보의 id를 찾아서 votes에 insert
      const voterId = profiles.find(p=>p.username===selectedProfile)?.id;
      const candidateId = profiles.find(p=>p.username===selectedCandidate)?.id;
      if(!voterId||!candidateId) return setStatus('프로필 ID를 찾을 수 없음');

      // 방 id는 임의로 rooms[0] 사용
      const roomId = rooms[0]?.id || 'test_room_auto';
      const { data: insData, error: insErr } = await supabase.from('votes').insert([{ room_id: roomId, voter_id: voterId, candidate_id: candidateId }]);
      if (insErr) {
  ...
  const loadCounts = async ()=>{
    try{
      ...
      if (votesErr) {
        ...
        setStatus('votes 로드 실패: ' + (votesErr.message || JSON.stringify(votesErr)));
        return;
      }
      const tally:any = {};
      if(votes) votes.forEach((v:any)=> tally[v.candidate_id] = (tally[v.candidate_id]||0)+1);
      setStatus(JSON.stringify(tally,null,2));
    }catch(e:any){ setStatus('로드 오류: '+(e.message||e)); }
  }
    ...
    <Container>
      ...
      <Card>
        <div style={{display:'flex',gap:16}}>
          <div style={{flex:1}}>
            <h4>프로필</h4>
            <select style={{width:'100%'}} value={selectedProfile||''} onChange={(e)=>setSelectedProfile(e.target.value||null)}>
              <option value="">-- 선택 --</option>
              {profiles.map(p=> <option key={p.id} value={p.username}>{p.username} ({p.realname||''})</option>)}
            </select>

            <h4 style={{marginTop:12}}>후보</h4>
            <select style={{width:'100%'}} value={selectedCandidate||''} onChange={(e)=>setSelectedCandidate(e.target.value||null)}>
              <option value="">-- 선택 --</option>
          ...
          <div style={{flex:1}}>
            <h4>방 목록</h4>
            <pre style={{whiteSpace:'pre-wrap'}}>{JSON.stringify(rooms,null,2)}</pre>
          </div>
        </div>

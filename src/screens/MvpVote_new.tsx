...
function MvpVote() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (allRoomsErr) {
          console.error('rooms select error:', allRoomsErr);
          return;
        }
        if (!allRooms) {
          console.log('No rooms found for yesterday');
          return;
        }
        
        // 사용자가 참여한 방들만 필터링
        const participatedRooms = allRooms.filter((room: any) => {
          const members = room.members ? (Array.isArray(room.members) ? room.members : JSON.parse(room.members)) : [];
          return members.includes(user.username);
        });
        
        console.log('User participated rooms:', participatedRooms);
        setUserRooms(participatedRooms);
        
        // 모든 참여 방의 멤버들 수집 (중복 제거)
        const allMembersSet = new Set<string>();
        participatedRooms.forEach((room: Room) => {
          const members = room.members ? (Array.isArray(room.members) ? room.members : JSON.parse(room.members as string)) : [];
        ...
        if (uniqueMembers.length > 0) {
          ...
          if (profilesErr) {
            console.error('profiles select error:', profilesErr);
          }
          ...
          const roomIds = participatedRooms.map((room: Room) => room.id);
          if (roomIds.length > 0) {
            ...
            if (votes) votes.forEach((r: any) => { 
              // candidate_id가 UUID이므로 username으로 변환 필요
              const username = Object.keys(profileMap).find(key => profileMap[key] === r.candidate_id);
              if (username) {
  ...
  const submitVote = async () => {
    ...
    try {
      ...
      const { data: voterProfile } = await supabase.from('profiles').select('id').eq('username', user.username).single();
      ...
      const { data: delData, error: delErr } = await supabase
        ...
        .eq('voter_id', voterId)
        .in('room_id', roomIds);
      if (delErr) console.error('delete existing votes error:', delErr, delData);

      // 가장 최근 방에 투표 기록 (또는 모든 방에 투표하려면 forEach 사용)
      const latestRoom = userRooms[userRooms.length - 1];
      const { data: insertData, error: insertErr } = await supabase.from('votes').insert([{ 
        room_id: latestRoom.id, 
        voter_id: voterId, 
        candidate_id: candidateId 
      }]);
      if (insertErr) console.error('insert vote error:', insertErr, insertData);
      
      // 투표 현황 새로고침
  const votesResp: any = await supabase
        .from('votes')
        .select('candidate_id')
        .in('room_id', roomIds);
  const votes = votesResp.data;
  const votesErr = votesResp.error;
  if (votesErr) console.error('refresh votes select error:', votesErr);
      
      const c: Record<string, number> = {};
      if (votes) votes.forEach((r: any) => { 
        const username = Object.keys(memberProfiles).find(key => memberProfiles[key] === r.candidate_id);
        if (username) {
          c[username] = (c[username] || 0) + 1;
        }
      });
  ...
  const finalize = async () => {
    ...
    for (const room of userRooms) {
      const { data: votes, error: votesErr } = await supabase
        ...
        .select('candidate_id')
        .eq('room_id', room.id);
      if (votesErr) console.error('votes select error for room', room.id, votesErr);
      if (votes) {
        allVotes.push(...votes);
      }
    }
    ...
    alert(`MVP: ${profile.username} (${profile.realname ?? '실명 없음'}) - 온도 10도 상승!`);
    navigate('/home');
  };

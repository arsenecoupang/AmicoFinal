...
function DebugDatabase() {
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'monospace' }}>
      ...
      <button onClick={checkDatabase} style={{ padding: '0.5rem 1rem', marginBottom: '2rem' }}>
        데이터베이스 스키마 확인
      </button>
      ...
      <div style={{ display: 'grid', gap: '2rem' }}>
        ...
        <div>
          ...
          <div style={{ border: '1px solid #ccc', padding: '1rem', backgroundColor: '#f9f9f9' }}>
            {tables.length === 0 ? (
              <div>아직 확인되지 않음</div>
            ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ border: '1px solid #ddd', padding: '8px' }}>Nullable</th>
                ...
                <tbody>
                  {chatsSchema.map((col, idx) => (
                    <tr key={idx}>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{col.column_name}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{col.data_type}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{col.is_nullable}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{col.column_default || 'NULL'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

...
function MvpVote() {
  ...
  const submitVote = async () => {
    ...
    try {
      ...
      await supabase.from('votes').delete().eq('voter_id', voterId).in('room_id', userRooms);
      
      // 가장 최근 방에 투표 (또는 모든 방에 투표하려면 userRooms.forEach 사용)
      const latestRoomId = userRooms[userRooms.length - 1];
      await supabase.from('votes').insert([{ room_id: latestRoomId, voter_id: voterId, candidate_id: candidateId }]);
      
      // refresh counts
      const { data: votes } = await supabase.from('votes').select('candidate_id').in('room_id', userRooms);
      const c: Record<string, number> = {};
      if (votes) votes.forEach((r:any) => { 

...
export function StoreProvider({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
		save({ users, mvpHistory, participation, votes });
	}, [users, mvpHistory, participation, votes]);
	...
	const addMessage = (username: string) => {
		setUsers((prev) =>
			prev.map((u) =>
				u.username === username
					? { ...u, temp: Math.min(100, u.temp + 0.5) }
					: u,
			),
		);
	};
	...
	const awardMvp = (userId: number, roomLabel: string) => {
		setUsers((prev) =>
			prev.map((u) =>
				u.id === userId
					? { ...u, temp: Math.min(100, u.temp + 20), revealed: true }
					: u,
			),
		);
		const rec: MvpRecord = {
	...
	const recordParticipation = (
		...
		setParticipation((prev) => {
			const exists = prev.some(
				(p) => p.userId === u.id && p.date === d && p.roomLabel === roomLabel,
			);
	...
	const getCandidates = (date: string, roomLabel?: string): StoreUser[] => {
		const setIds = new Set(
			participation
				.filter(
					(p) => p.date === date && (!roomLabel || p.roomLabel === roomLabel),
				)
				.map((p) => p.userId),
		);
	...
	const voteMvp = (
		...
		setVotes((prev) => {
			const already = prev.some(
				(x) =>
					x.date === date && x.roomLabel === roomLabel && x.voter === voter,
			);
	...
	const hasVoted = (date: string, roomLabel: string, voter: string) =>
		votes.some(
			(v) => v.date === date && v.roomLabel === roomLabel && v.voter === voter,
		);

	const getMvpTally = (date: string, roomLabel: string) => {
		const map = new Map<number, number>();
		votes
			.filter((v) => v.date === date && v.roomLabel === roomLabel)
			.forEach((v) => {
	...
	const finalizeMvp = (date: string, roomLabel: string): StoreUser | null => {
		// 이미 발표되었는지 검사
		const exists = mvpHistory.some(
			(h) => h.date === date && h.roomLabel === roomLabel,
		);
		if (exists) {
			const last = mvpHistory.find(
				(h) => h.date === date && h.roomLabel === roomLabel,
			)!;

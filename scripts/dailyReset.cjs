...
const supabase = !DRY_RUN && SUPABASE_URL && SUPABASE_KEY ? createClient(SUPABASE_URL, SUPABASE_KEY) : null;

function cryptoRandomUUID() {
  try { return globalThis.crypto?.randomUUID?.() ?? require('crypto').randomUUID(); } catch { return require('crypto').randomBytes(16).toString('hex'); }
}

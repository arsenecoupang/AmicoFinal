...
async function run() {
	...
	try {
		...
		const systemMsg =
			'"너는 유행할만한 밸런스 게임 질문을 생성하는 역할을 한다. 출력은 반드시 JSON 형식으로만 하며, 불필요한 설명, 코드 블록, 마크다운은 절대 포함하지 않는다. 형식은 다음과 같다: {"question": "질문", "option1": "선택지1", "option2": "선택지2"}. 질문은 트렌디하고 사람들이 흥미를 느낄 수 있는 주제로 구성한다."';
		const userPrompt = "새로운 밸런스 게임 질문 하나를 생성해줘.";
		...
		if (OPENAI_API_KEY && ENABLE_OPENAI) {
			...
			for (let attempt = 1; attempt <= 3; attempt++) {
				try {
					...
					if (
						...
						console.warn(
							`GPT API quota/rate error (status ${res.status}, code ${json?.error?.code}) on attempt ${attempt}`,
						);
						parsed = null;
			...
			if (!ENABLE_OPENAI)
				...
				console.log(
					"🔧 OPENAI_API_KEY not set. Using local fallback questions.",
				);
			useFallback();
		}
